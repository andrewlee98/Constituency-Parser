...
for file in os.listdir('data/features/'):
    if file[0] == '.': continue
    elif file[0:2] in {'22'}: val_data.extend(pickle.load(open('data/features/' + file, 'rb'))) # use folder 22 as validation set
    elif file[0:2] in {'23'}: test_data.extend(pickle.load(open('data/features/' + file, 'rb'))) # use folder 23 for test
    else: train_data.extend(pickle.load(open('data/features/' + file, 'rb'))) # training data
